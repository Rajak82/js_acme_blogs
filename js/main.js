...
function createElemWithText(para1, para2, para3) {
...
const element1 = document.createElement(para1)
element1.textContent = para2;

if(para3 != undefined){
element1.classList.add(para3)
}
...
if(userData == undefined){
    ...
    const jsonUserData = userData;
    const elementArray = []

    jsonUserData.forEach(element => {
        const options = document.createElement("option")
        options.value = element.id
        options.textContent = element.name
        elementArray.push(options)
...
function toggleCommentSection(postID){
    ...
    const element1 = document.querySelectorAll('section')
    for(let i = 0; i<element1.length;i++){
        const element2 = element1[i]

        if(element2.getAttribute('data-post-id') == postID){
...
function toggleCommentButton(postID){
  ...
  const element1 = document.querySelectorAll('button')
    for(let i = 0; i<element1.length;i++){
        const element2 = element1[i]

        if(element2.getAttribute('data-post-id') == postID){
            
            element2.textContent == "Show Comments"
        ? (element2.textContent = "Hide Comments")
        : (element2.textContent = "Show Comments");
...
function addButtonListeners(){
    ...
    const buttons = element1.querySelectorAll('button')
...
function createComments(jsonComments){
  ...
  const jComments = jsonComments;

  jComments.forEach(comment =>{
    const element1 = document.createElement('article');
    const element2 = createElemWithText('h3', comment.name)
    const element3 = createElemWithText('p', comment.body)
    const element4 = createElemWithText('p', `From: ${comment.email}`)
    element1.append(element2)
    element1.append(element3)
    element1.append(element4)
    dFrag.append(element1)
  })
...
function populateSelectMenu(jsonUserData){
    ...
    const element1 = createSelectOptions(jsonUserData)

    element1.forEach(option => {
...
if (!userID) {
    ...
    const result = jsonUserPosts.filter(element => (element.userId == userID))
...
const getUser =  async (userID) =>{
    ...
    const response = await fetch("https://jsonplaceholder.typicode.com/users");
    const jsonUserData= await response.json();
    const result = jsonUserData.find(element => (element.id == userID))
...
const getPostComments = async (postID) =>{
    ...
    const jsonPostComment= await response.json();
    const result = jsonPostComment.filter(element => (element.postId == postID))
...
pElement2.textContent = `Post ID: ${post.id}`

const author = await getUser(post.userId)

const pElement3 = document.createElement('p')
pElement3.textContent = `Author: ${author.name} with ${author.company.name}`

const pElement4 = document.createElement('p')
pElement4.textContent = author.company.catchPhrase

const button = document.createElement('button')
button.textContent = 'Show Comments'
button.setAttribute('data-post-Id', post.id)

article.append(h2Element, pElement, pElement2, pElement3, pElement4, button)
...
const main = document.querySelector('main')
    ...
    if (!posts) {
       const element = main.querySelector('.default-text')
        ...
        element = await createPosts(newPosts)
        main.append(element)
        return element
    }
}
...
function toggleComments(e, postId){
    ...
    const element1 = toggleCommentSection(postId)
    const element2 = toggleCommentButton(postId)
...
const main = deleteChildElements(element1)
...
const myArray = [userId, posts, refreshPostsArray]
